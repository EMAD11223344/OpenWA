...
- **Engine consolidation**: whatsapp-web.js is now the only WhatsApp engine. Removed
  the alternative engine plugin, adapter, and dependency. `ENGINE_TYPE` accepts only
  `whatsapp-web.js` (default).
- **Engine factory**: Default engine is now `whatsapp-web.js`; unknown engine types
  fall back to the whatsapp-web.js adapter.
- **Dashboard**: Engine selector removed everywhere; UI reflects whatsapp-web.js only.
- **Docs/changelog**: Historical references to the removed engine scrubbed.
